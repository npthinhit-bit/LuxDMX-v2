...
## BUG-024: Blank web page in browser — sendAppPage() missing Content-Encoding: gzip header
...
treats the gzipped binary payload as plain text — the gzipped bytes are not
...
- Adding a `Vary: Accept-Encoding` header without `Content-Encoding`: this
  ...
  build time — the server must tell the browser the encoding it is sending.
...
`sendRawPage()` was intentionally **not** modified — it serves raw (non-gzipped)
HTML for the setup, reset, OTA-progress, and config-saved pages. Adding the
gzip header to those would break them.

### Files / Functions
- `src/frontend/web_frontend.cpp` — `sendAppPage()` (added line 72:
...
timing-critical DMX/RDM/TTL paths are touched — this is purely an HTTP response
...
a silent blank page — a particularly insidious failure mode because the HTTP
