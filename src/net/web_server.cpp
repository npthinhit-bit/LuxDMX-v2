...
static void rateLimitHandler(AsyncWebServerRequest* req, ArRequestHandlerFunction handler, RateLimiter& rl)
{
...
void webRegisterRoutes(AsyncWebServer& http)
{
    ...
    http.on("/ota/github", HTTP_POST,
            [](AsyncWebServerRequest* req)
            {
                rateLimitHandler(req, handleOtaGithub, g_otaRateLimiter);
            });
    http.on("/ota/url", HTTP_POST,
            [](AsyncWebServerRequest* req)
            {
