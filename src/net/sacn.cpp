...
static uint16_t sacnUniverseFor(int outIdx)
{
    ...
    if (su <= 0)
        su = cfg.outputs[outIdx].universe + 1;
    return (uint16_t)su;
}
...
{
    if (!cidMutex)
        cidMutex = xSemaphoreCreateMutex();
}
...
static void sendSacnDiscovery()
{
    ...
    for (int page = 0; page < pages; page++)
    {
        ...
        if (count > (int)SACN_DISC_MAX_UNIV)
            count = (int)SACN_DISC_MAX_UNIV;
...
static void handleSacnPacket(int outIdx, uint32_t senderIp, const uint8_t* p, int n)
{
    ...
    if (n < SACN_MIN_LEN)
        return;
    if (memcmp(p + SACN_ACN_ID_OFF, ACN_PACKET_ID, 12) != 0)
        return;

    uint32_t rootVec = ((uint32_t)p[SACN_ROOT_VEC_OFF] << 24) | ((uint32_t)p[SACN_ROOT_VEC_OFF + 1] << 16) |
                       ((uint32_t)p[SACN_ROOT_VEC_OFF + 2] << 8) | (uint32_t)p[SACN_ROOT_VEC_OFF + 3];
    if (rootVec != 0x00000004u)
        return;

    uint32_t frameVec = ((uint32_t)p[SACN_FRAME_VEC_OFF] << 24) | ((uint32_t)p[SACN_FRAME_VEC_OFF + 1] << 16) |
                        ((uint32_t)p[SACN_FRAME_VEC_OFF + 2] << 8) | (uint32_t)p[SACN_FRAME_VEC_OFF + 3];

    // Universe Discovery (receiver consumes/discards)
    if (frameVec == SACN_FRAME_VEC_DISCOVERY)
        return;

    // Standard streaming data
    if (frameVec != SACN_FRAME_VEC_STREAM)
        return;

    uint16_t universe = ((uint16_t)p[SACN_UNIVERSE_OFF] << 8) | p[SACN_UNIVERSE_OFF + 1];

    if (p[SACN_STARTCODE_OFF] != 0x00)
        return;
...
void readSacnSocket(int outIdx)
{
    ...
    for (int guard = 0; guard < 4; guard++)
    {
        ...
        if (n <= 0)
            continue;
        if (n < SACN_MIN_LEN)
            continue;  // too short to be a valid sACN frame
...
void readSacn()
{
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        ...
        if (dmxBufferState().sacnSyncAddr[i] != 0 && dmxBufferState().sacnStagedValid[i])
        {
            ...
            if (syncMs >= 2500)
            {
                routeFrame((int)portAddress(cfg.outputs[i]), dmxBufferState().sacnStaged[i],
                           dmxBufferState().sacnStagedLen[i], 0, 1, DEFAULT_PRIORITY);
    ...
    if (sacnSyncUdp.available())
    {
        ...
        if (n >= SACN_MIN_LEN)
        {
            ...
            if (r >= SACN_MIN_LEN && memcmp(sacnBuf + SACN_ACN_ID_OFF, ACN_PACKET_ID, 12) == 0)
            {
                uint32_t fvec =
                    ((uint32_t)sacnBuf[SACN_FRAME_VEC_OFF] << 24) | ((uint32_t)sacnBuf[SACN_FRAME_VEC_OFF + 1] << 16) |
                    ((uint32_t)sacnBuf[SACN_FRAME_VEC_OFF + 2] << 8) | (uint32_t)sacnBuf[SACN_FRAME_VEC_OFF + 3];
                if (fvec == SACN_FRAME_VEC_SYNC)
                {
                    uint16_t su = ((uint16_t)sacnBuf[SACN_UNIVERSE_OFF] << 8) | sacnBuf[SACN_UNIVERSE_OFF + 1];
                    for (int i = 0; i < MAX_OUTPUTS; i++)
                    {
