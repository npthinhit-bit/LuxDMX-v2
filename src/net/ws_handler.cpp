...
void handleWsText(const char* payload, size_t len, uint32_t clientId)
{
    ...
    if (msg.indexOf("\"viewout\"") >= 0)
    {
        ...
        if (k >= 0)
        {
            ...
            if (o >= 0 && o < MAX_OUTPUTS && cfg.outputs[o].enabled)
                monitorOut = o;
        }
        return;
    }
    if (msg.indexOf("\"blackout\"") >= 0)
    {
        ...
        dmxBufWriteBegin(mo);
        memset(&dmxBufferState().buffers[mo].data[1], 0, 512);
        dmxBufWriteEnd(mo);
        return;
    }
    ...
    if (msg.indexOf("\"scene\"") >= 0)
    {
        ...
        uint16_t fadeMs   = 0;
        if (fadeIdx >= 0)
            fadeMs = (uint16_t)msg.substring(fadeIdx + 6).toInt();
