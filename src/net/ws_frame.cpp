...
void wsBuildFrame()
{
    ...
    {
        int s = ETH.linkSpeed();
        rssi  = (int16_t)(s >= 10 ? s : 100);
    }
    else
        rssi = (int16_t)WiFi.RSSI();
    ...
    wsBuf[0]  = fpsI >> 8;
    wsBuf[1]  = fpsI & 0xFF;
    wsBuf[2]  = (uint8_t)((uint16_t)rssi >> 8);
    wsBuf[3]  = rssi & 0xFF;
    wsBuf[4]  = heap >> 24;
    wsBuf[5]  = (heap >> 16) & 0xFF;
    wsBuf[6]  = (heap >> 8) & 0xFF;
    wsBuf[7]  = heap & 0xFF;
    wsBuf[8]  = upS >> 24;
    wsBuf[9]  = (upS >> 16) & 0xFF;
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        if (nowMs - stats().inWinMs[i] >= 1000)
        {
            ...
            stats().inFrameCnt[i] = 0;
            stats().inWinMs[i]    = nowMs;
        }
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        ...
        if (dmxIsDelta(i))
            st |= 0x01;
        if (cfg.outputs[i].txStyleSrc == TXSRC_ARTNET)
            st |= 0x02;
    ...
    wsBuf[t + 1] = nf & 0xFF;
    wsBuf[t + 2] = rtx >> 24;
    wsBuf[t + 3] = (rtx >> 16) & 0xFF;
    wsBuf[t + 4] = (rtx >> 8) & 0xFF;
    wsBuf[t + 5] = rtx & 0xFF;
    wsBuf[t + 6] = rrx >> 24;
    wsBuf[t + 7] = (rrx >> 16) & 0xFF;
    wsBuf[t + 8] = (rrx >> 8) & 0xFF;
    wsBuf[t + 9] = rrx & 0xFF;
}
