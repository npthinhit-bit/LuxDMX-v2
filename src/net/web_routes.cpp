...
void handleDmxJson(AsyncWebServerRequest* req)
{
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        ...
        j += char('A' + i);
        j += "\",\"universe\":";
        j += cfg.outputs[i].universe;
        j += ",\"enabled\":";
        j += cfg.outputs[i].enabled ? "true" : "false";
...
{
    bool includeCreds =
        req->hasParam("include_credentials", true) && req->getParam("include_credentials", true)->value() == "1";
    String j;
    cfgcore::exportJson(j, !includeCreds);
    sendJson(req, j);
}
...
void handleConfigPost(AsyncWebServerRequest* req)
{
    ...
    for (int o = 0; o < MAX_OUTPUTS; o++)
    {
        for (size_t i = 0; i < OUTPUT_FIELD_COUNT; i++)
        {
            ...
            if (!req->hasParam(key.c_str(), true))
                continue;
...
void handleRdmTrigger(AsyncWebServerRequest* req)
{
    ...
    if (action == "setaddr" || action == "identify" || action == "setpers" || action == "setlabel")
    {
        rdm_uid_t uid;
