...
#include "net_state.h"  // startWiFiAP, startWiFiStation, g_* flags
...
{
    uint32_t t0 = millis();
    while (!ETH.linkUp() && millis() - t0 < 15000)
        delay(50);
    if (ETH.linkUp())
...
void applyEthStaticIp()
{
    ...
    parseIp(cfg.ip, ip);
    parseIp(cfg.gateway, gw);
    parseIp(cfg.subnet, sn);
    parseIp(cfg.dns, dns);
...
static void ethSpiUpTask(void*)
{
    ...
    if (isW5500)
        w5500HardReset();
    ETH.begin(phy, ETH_W5500_ADDR, cfg.ethCs, cfg.ethInt, isW5500 ? -1 : cfg.ethRst, ETH_W5500_SPI_HOST, cfg.ethSck,
              cfg.ethMiso, cfg.ethMosi, cfg.ethFreqMhz);
...
static void startEthSpi()
{
    Serial.printf("[ETH] %s SPI cs=%d irq=%d rst=%d sck=%d miso=%d mosi=%d freq=%dMHz\n",
                  cfg.ethSpiPhy == ETH_SPI_PHY_DM9051 ? "DM9051" : "W5500", cfg.ethCs, cfg.ethInt, cfg.ethRst,
                  cfg.ethSck, cfg.ethMiso, cfg.ethMosi, cfg.ethFreqMhz);
    s_ethUpDone = false;
    xTaskCreatePinnedToCore(ethSpiUpTask, "ethup", 8192, nullptr, 5, nullptr, 0);
    uint32_t t0 = millis();
    while (!s_ethUpDone && millis() - t0 < 30000)
        delay(20);
    if (!s_ethUpDone)
        Serial.println("[ETH] SPI bring-up still running after 30s");
}
#endif  // HAS_ETH_SPI
...
static void ethRmiiUpTask(void*)
{
    ...
    ETH.begin(rmiiPhyType(phy), cfg.rmiiAddr, cfg.rmiiMdc, cfg.rmiiMdio, cfg.rmiiPwr, rmiiClkMode(cfg.rmiiClk));
    ETH.setHostname(cfg.hostname.c_str());
    applyEthStaticIp();
    waitEthLink();
...
{
    int phy = constrain(cfg.rmiiPhy, 0, RMII_PHY_COUNT - 1);
    Serial.printf("[ETH] %s RMII addr=%d mdc=%d mdio=%d pwr=%d clk=%d\n", RMII_PHY_NAMES[phy], cfg.rmiiAddr,
                  cfg.rmiiMdc, cfg.rmiiMdio, cfg.rmiiPwr, cfg.rmiiClk);
    s_ethRmiiUpDone = false;
    xTaskCreatePinnedToCore(ethRmiiUpTask, "ethrmii", 8192, nullptr, 5, nullptr, 0);
    uint32_t t0 = millis();
    while (!s_ethRmiiUpDone && millis() - t0 < 30000)
        delay(20);
    if (!s_ethRmiiUpDone)
        Serial.println("[ETH] RMII core-0 bring-up still running after 30s");
}
#endif
...
void startWiredEth()
{
    ...
    if (cfg.wiredPhy == WIRED_PHY_RMII)
        startEthRmii();
    else
        startEthSpi();
