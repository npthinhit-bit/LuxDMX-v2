...
void wsPush() {
    ...
    for (int i = 0; i < WS_MAX_CLIENTS; i++) {
        ...
        if (!c) continue;
        if (c->status() != WS_CONNECTED) { wsClients[i] = nullptr; continue; }
        ...
        if (changed == 0 && wsClientFrameSeq[i] == wsFrameSeq) continue;
        wsClientFrameSeq[i] = wsFrameSeq;
        if (!c->canSend()) continue;
        try { c->binary((const char*)wsBuf, WS_FRAME_LEN); } catch (...) {}
    }
}
...
void wsInit(AsyncWebServer& srv) {
    ws.onEvent([](AsyncWebSocket* server, AsyncWebSocketClient* client,
        ...
        if (type == WS_EVT_CONNECT) {
            ...
            if (wsClients[slot] == client) wsClients[slot] = nullptr;
            wsClientSub[slot] = 0;
