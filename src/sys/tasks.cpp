...
void createTasks()
{
    ...
    if (dispReady)
        xTaskCreate(displayTask, "disp", 4096, nullptr, 1, nullptr);
...
static void snapshotAndTransmit(int outIdx)
{
    if (!outReady[outIdx])
        return;
    if (cfg.outputs[outIdx].txStyle == TXSTYLE_DELTA && !stats().outSrcLost[outIdx])
        return;
    uint8_t frame[DMX_PACKET_SIZE];
    if (!dmxBufSnapshot(outIdx, frame))
        return;
...
static void dmxFrameTick()
{
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        ...
        {
            if (cfg.outputs[i].txStyle == TXSTYLE_DELTA)
                anyDelta = true;
