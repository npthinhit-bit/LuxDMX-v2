...
struct NavQueue
{
    ...
    void     push(NavEvent e)
    {
        ...
        if (n == head)
            return;  // full: drop oldest-safe (shouldn't happen in practice)
        ev[tail] = e;
        tail     = n;
    }
    NavEvent pop()
...
struct InputMapper
{
    ...
    InputMapper() {}
    explicit InputMapper(const InputConfig& c)
    {
        begin(c);
    }
    ...
    void begin(const InputConfig& c)
    {
        ...
        for (int i = 0; i < INPUT_MAX_BTN; i++)
            if (c.btnPresent[i] && c.role[i] != ROLE_OFF)
                actuators++;
    ...
    bool canSelect() const
    {
        if (cfg.hasSw)
            return true;
        for (int i = 0; i < INPUT_MAX_BTN; i++)
            if (cfg.btnPresent[i] && cfg.role[i] != ROLE_OFF)
                return true;
        return false;
    }
    bool hasAnyInput() const
    {
        return cfg.hasEncoder || canSelect();
    }

    // logical "pressed" from a raw level, honouring the wiring choice.
    bool pressed(bool rawLevel) const
    {
        return cfg.activeHigh ? rawLevel : !rawLevel;
    }
    ...
    void poll(const InputSample& s, uint32_t now)
    {
        ...
        if (cfg.hasSw && s.swPresent)
        {
            ...
            if (soloButton)
            {
                if (p == ENC_PRESS_SHORT)
                    q.push(NAV_INC);
                else if (p == ENC_PRESS_LONG)
                    q.push(NAV_ENTER);
            }
            else
            {
        ...
        for (int i = 0; i < INPUT_MAX_BTN; i++)
        {
            if (!(cfg.btnPresent[i] && s.btnPresent[i]) || cfg.role[i] == ROLE_OFF)
                continue;
            EncPress p = btn[i].feed(pressed(s.btnLevel[i]), now);
            if (p == ENC_PRESS_NONE)
                continue;
            if (soloButton)
            {  // lone button: short=INC, long=ENTER
...
private:
    ...
    static NavEvent mapRole(BtnRole r, EncPress p)
    {
        ...
        {
        case ROLE_NEXT:
            return lng ? NAV_ENTER : NAV_INC;  // long-press commits
        case ROLE_PREV:
            return lng ? NAV_ENTER : NAV_DEC;
        case ROLE_ENTER:
            return lng ? NAV_BACK : NAV_ENTER;  // long-press backs out
        case ROLE_BACK:
            return NAV_BACK;
        default:
            return NAV_NONE;
