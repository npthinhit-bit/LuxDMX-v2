...
struct Menu
{
    ...
    void addValue(uint8_t id, const char* label, int32_t v, int32_t mn, int32_t mx, bool enabled = true)
    {
        ...
        strncpy(it.label, label, MENU_LABEL_LEN - 1);
        it.label[MENU_LABEL_LEN - 1] = 0;
        it.id                        = id;
        it.kind                      = MI_VALUE;
        it.enabled                   = enabled;
        it.value                     = v;
    ...
    {
        MenuResult r;
        if (mode == MENU_CLOSED || e == NAV_NONE)
            return r;
        ...
        if (mode == MENU_BROWSE)
        {
            ...
            case NAV_ENTER:
                ...
                {
                    r.actionId = items[sel].id;
                    r.changed  = true;
                }
                else
                {
                    mode      = MENU_EDIT;
                    edit      = items[sel].value;
                    r.changed = true;
                }
                break;
            case NAV_BACK:
        ...
        {
        case NAV_INC:
            edit      = wrap(edit + 1, it.vmin, it.vmax);
            r.changed = true;
            break;
        case NAV_DEC:
            edit      = wrap(edit - 1, it.vmin, it.vmax);
