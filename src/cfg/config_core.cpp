...
static void* rootAddr(const CfgField& f)
{
    return (char*)&cfg + f.offset;
}
static void* outAddr(int i, const CfgOutputField& f)
{
    return (char*)&cfg.outputs[i] + f.offset;
}

static int neutralInt(int32_t mn)
{
    return mn <= -1 ? -1 : (int)mn;
}
...
esp_err_t setValue(const String& key, const String& val, String& err)
{
    ...
    int32_t  mn, mx;
    uint16_t flags;
    if (!resolve(key, a, kind, mn, mx, flags))
    {
        ESP_LOGE("cfgcore", "unknown key: %s", key.c_str());
...
esp_err_t getValue(const String& key, String& out)
{
    ...
    if (!resolve(key, a, kind, mn, mx, flags))
    {
        ESP_LOGE("cfgcore", "key not found: %s", key.c_str());
        return ESP_ERR_INVALID_ARG;
    }
...
static void applyNeutral()
{
    for (size_t j = 0; j < CONFIG_FIELD_COUNT; j++)
    {
        ...
        if (f.kind == CfgKind::Bool)
            *(bool*)a = false;
        else if (f.kind == CfgKind::Str)
            *(String*)a = "";
        else
            *(int*)a = neutralInt(f.min);
    }
...
esp_err_t applyTemplateText(const char* text, String& err, int depth)
{
    if (depth > 8)
    {
        ESP_LOGE("cfgcore", "template nesting too deep (>%d)", depth);
        err = "template nesting too deep";
    ...
    while (*p)
    {
        ...
        *eq      = 0;
        ...
        if (strcmp(k, "extends") == 0)
        {
            if (applyNamed(String(v), err, depth + 1) != ESP_OK)
                return ESP_ERR_INVALID_ARG;
            continue;
        }
        String e2;
        if (setValue(String(k), String(v), e2) != ESP_OK)
        {
            err = e2;
            return ESP_ERR_INVALID_ARG;
        }
...
void resetToTemplate()
{
    applyNeutral();
    String err;
    applyTemplate(CFG_STR(DEFAULT_TEMPLATE), err);
}
...
void load()
{
    ...
    Preferences prefs;
    prefs.begin(CFG_PREF_NS, false);
    for (size_t j = 0; j < CONFIG_FIELD_COUNT; j++)
    {
        const CfgField& f = CONFIG_FIELDS[j];
        void*           a = rootAddr(f);
        if (f.kind == CfgKind::Bool)
            *(bool*)a = prefs.getBool(f.key, *(bool*)a);
        else if (f.kind == CfgKind::Str)
        {
            if (prefs.isKey(f.key))
                *(String*)a = prefs.getString(f.key, *(String*)a);
        }
        else
        {
            int v    = prefs.getInt(f.key, *(int*)a);
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
        for (size_t j = 0; j < OUTPUT_FIELD_COUNT; j++)
        {
            ...
            if (i < 2 && !prefs.isKey(key.c_str()))
            {
                ...
                if (prefs.isKey(legacy.c_str()))
                {
                    ...
                    else
                    {
                        int base = *(int*)a;
                        if (f.legacyKey0 && i == 0)
                            base = prefs.getInt(f.legacyKey0, base);
            ...
            if (f.kind == CfgKind::Bool)
            {
                *(bool*)a = prefs.getBool(key.c_str(), *(bool*)a);
                continue;
            }
            int base = *(int*)a;
            if (f.legacyKey0 && i == 0)
...
void save()
{
    ...
    for (size_t j = 0; j < CONFIG_FIELD_COUNT; j++)
    {
        ...
        void*           a = rootAddr(f);
        if (f.kind == CfgKind::Bool)
            prefs.putBool(f.key, *(bool*)a);
        else if (f.kind == CfgKind::Str)
            prefs.putString(f.key, *(String*)a);
        else
            prefs.putInt(f.key, *(int*)a);
    }
    for (int i = 0; i < MAX_OUTPUTS; i++)
        for (size_t j = 0; j < OUTPUT_FIELD_COUNT; j++)
        {
            ...
            String                key = outKey(i, f.suffix);
            if (f.kind == CfgKind::Bool)
                prefs.putBool(key.c_str(), *(bool*)a);
            else
                prefs.putInt(key.c_str(), *(int*)a);
        }
...
void dump(String& out, bool maskSecrets)
{
    for (size_t j = 0; j < CONFIG_FIELD_COUNT; j++)
    {
        ...
        String          v;
        getValue(f.key, v);
        if (maskSecrets && (f.flags & CFG_SECRET))
            v = "***";
        out += f.key;
        out += "=";
        out += v;
...
esp_err_t importJson(const String& json, String& err)
{
    ...
    if (!ok)
    {
        ESP_LOGE("cfgcore", "importJson: some keys not recognized");
        err = "some keys not recognized";
    }
...
esp_err_t importXml(const String& xml, String& err)
{
    ...
    while (pos < len)
    {
        ...
        if (tag.startsWith("/") || tag.startsWith("?") || tag.startsWith("config") || tag.startsWith("global") ||
            tag.startsWith("outputs") || tag.startsWith("output "))
        {
            pos = tagEnd + 1;
            continue;
        }
        ...
        val.trim();
        String e2;
        if (setValue(tag, val, e2) != ESP_OK)
            ok = false;
        pos = closeTag + tag.length() + 3;
    }
    if (!ok)
    {
        ESP_LOGE("cfgcore", "importXml: some keys not recognized");
        err = "some XML keys not recognized";
    }
    return ok ? ESP_OK : ESP_ERR_INVALID_ARG;
}
