...
static const char* const ENUM_TXRATE[]    = {"40 fps (25 ms)", "41.7 fps (24 ms)", "33.3 fps (30 ms)", "25 fps (40 ms)",
                                             "20 fps (50 ms)"};
...
#define EFIELD(key, json, member, label, group, labels)                                                \
    {key,   json,       CfgKind::Enum, offsetof(Config, member), 0, (int32_t)ARRSZ(labels) - 1, label, \
     group, CFG_REBOOT, labels,        (uint8_t)ARRSZ(labels)}
...
#define EFIELD_L(key, json, member, label, group, labels)                                            \
    {key,   json,     CfgKind::Enum, offsetof(Config, member), 0, (int32_t)ARRSZ(labels) - 1, label, \
     group, CFG_LIVE, labels,        (uint8_t)ARRSZ(labels)}
...
const CfgField CONFIG_FIELDS[] = {
    ...
    BFIELD_L("autoip", "autoIpFallback", autoIpFallback, "AutoIP (169.254.x.x) fallback when DHCP fails", "Network",
             CFG_NONE),
...
#define OENUM_RO(suffix, json, member, label, labels) \
    {suffix,                                          \
     json,                                            \
     CfgKind::Enum,                                   \
     offsetof(DmxOutput, member),                     \
     nullptr,                                         \
     0,                                               \
     (int32_t)ARRSZ(labels) - 1,                      \
     label,                                           \
     (uint16_t)(CFG_LIVE | CFG_NOWEB),                \
     labels,                                          \
     (uint8_t)ARRSZ(labels)}
