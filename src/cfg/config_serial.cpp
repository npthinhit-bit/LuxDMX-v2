...
static void appendChar(String& s, char c)
{
    char b[2] = {c, 0};
    s += b;
}
...
static String applyKv(const String& line)
{
    ...
    while (i < n)
    {
        ...
        if (eq < 0)
        {
            fail++;
            if (!firstErr.length())
                firstErr = String("bad token: ") + tok;
            continue;
        }
        String k = tok.substring(0, eq), v = tok.substring(eq + 1);
        String e;
        if (cfgcore::setValue(k, v, e) == ESP_OK)
            ok++;
        else
        {
            fail++;
    ...
    if (fail)
        return String("ERR ") + firstErr + " (" + ok + " ok, " + fail + " failed)";
    return String("OK ") + ok;
}
...
String execute(const String& line)
{
    ...
    {
        String v;
        if (rest.length() && cfgcore::getValue(rest, v) == ESP_OK)
            return rest + "=" + v;
    ...
    if (verb == "wifi")
    {
        ...
        if (g_hooks.wifi && g_hooks.wifi(ssid, pass))
            return String("OK wifi set, connecting to ") + ssid;
