...
static void rdmTaskLoop(void* /*arg*/)
{
    ...
    while (g_rdmTask.running)
    {
        if (xQueueReceive(g_rdmTask.cmdQueue, &cmd, pdMS_TO_TICKS(10)) == pdTRUE)
        {
            ...
            {
                uint8_t pkt[64];
                int len = rdmBuild(pkt, RDM_UID_BROADCAST_ALL, RDM_CC_DISC_COMMAND, RDM_PID_DISC_UN_MUTE, nullptr, 0);
                rdmTx(pkt, len);
            ...
            {
                if (cmd.artReqLen && cmd.lineIdx >= 0)
                    rdmRmtSelect(cmd.lineIdx);
                RmtDmx* rd = g_rdm.rmt;
                if (rd && rd->chan)
                {
                    ...
                    if (reqLen >= RDM_HDR_LEN - 1 && reqLen <= 260)
                    {
                        ...
                        for (int attempt = 0; attempt < (bcast ? 1 : 3); attempt++)
                        {
                            ...
                            if (bcast)
                            {
                                if (!cmd.artReqLen && cmd.rawRelayResult)
                                    *cmd.rawRelayResult = 0;
                                success = true;
                                break;
                            }
                            uint8_t rx[96];
                            ...
                            int s = -1;
                            for (int i = 0; i < n - 1; i++)
                                if (rx[i] == RDM_SC && rx[i + 1] == RDM_SC_SUB)
                                {
                                    s = i;
                                    break;
                                }
                            ...
                            int      avail  = n - s;
                            int      msgLen = m[2];
                            if (msgLen + 2 > avail || msgLen < RDM_HDR_LEN)
                            {
                                esp_rom_delay_us(1000);
                                continue;
                            }
                            ...
                            for (int i = 0; i < msgLen; i++)
                                ck += m[i];
                            if (ck != (uint16_t)((m[msgLen] << 8) | m[msgLen + 1]))
                            {
                                esp_rom_delay_us(1000);
                                continue;
                            }
                            ...
                            {
                                if (outLen > cmd.respNoSCMax)
                                    outLen = cmd.respNoSCMax;
                                memcpy(cmd.respNoSC, m + 1, outLen);
                                if (cmd.rawRelayResult)
                                    *cmd.rawRelayResult = outLen;
                            }
                            success = true;
                            break;
                        }
...
bool rdmTaskInit()
{
    ...
    BaseType_t res = xTaskCreatePinnedToCore(rdmTaskLoop, "rdm_task", RDM_TASK_STACK_SIZE, nullptr, RDM_TASK_PRIORITY,
                                             &g_rdmTask.taskHandle, 1);
...
bool rdmTransactionAsync(const rdm_uid_t& dest, uint8_t cc, uint16_t pid, const uint8_t* reqPd, uint8_t reqPdl,
                         ...
                         bool* result)
{
    if (!g_rdmTask.running || !g_rdmTask.cmdQueue)
        return false;
...
bool rdmArtRawRelayEnqueue(const uint8_t* reqNoSC, uint16_t reqLen, uint32_t destIp, int lineIdx)
{
    ...
    if (reqLen > (uint16_t)sizeof(cmd.artReq))
        reqLen = (uint16_t)sizeof(cmd.artReq);
    if (reqLen > 0 && reqNoSC)
    {
...
int rdmRmtDiscover(rdm_uid_t* out, int max)
{
    ...
    int limit = max;
    if (cfg.rdmMaxDev > 0 && cfg.rdmMaxDev < limit)
        limit = cfg.rdmMaxDev;
