...
inline int rdmLineCount()
{
    return g_rdm.lineN;
}
inline uint32_t rdmSent()
{
    return g_rdm.sent;
}
inline uint32_t rdmRecv()
{
    return g_rdm.recv;
}
inline uint32_t rdmSentMs()
{
    return g_rdm.sentMs;
}
inline uint32_t rdmRecvMs()
{
    return g_rdm.recvMs;
}
inline uart_port_t rdmActiveUart()
{
    return g_rdm.uart;
}
...
int  rdmBuild(uint8_t* buf, const rdm_uid_t& dest, uint8_t cc, uint16_t pid, const uint8_t* pd, uint8_t pdl);
