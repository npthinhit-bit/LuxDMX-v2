...
{
    if (outIdx < 0 || outIdx >= MAX_OUTPUTS)
        return;
...
void outputInitAll()
{
    ...
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        ...
        bool dup = false;
        for (int j = 0; j < i; j++)
            if (outReady[j] && cfg.outputs[j].port == cfg.outputs[i].port && cfg.outputs[j].rtsPin >= 0 &&
                cfg.outputs[i].rtsPin >= 0)
                dup = true;
        if (dup)
        {
        ...
        {
            if (cfg.outputs[i].port == 1)
                uart = UART_NUM_1;
            else if (cfg.outputs[i].port == 2)
                uart = UART_NUM_2;
        }
        int rmtCh = i;
        if (rmtDmxInit(&g_outputs[i].rmt, cfg.outputs[i].txPin, rmtCh))
        {
            ...
            if (cfg.outputs[i].inputMode != DMX_IN_OFF && cfg.outputs[i].rxPin >= 0)
            {
                ...
                {
                    Serial.printf("[DMX] out%d input mode %d on uart%d rx=%d\n", i, cfg.outputs[i].inputMode,
                                  cfg.outputs[i].port, cfg.outputs[i].rxPin);
                }
                else
                {
            ...
            if (mode == OUTPUT_MODE_RDM_FULL)
            {
                int line = rdmRmtInit(&g_outputs[i].rmt, cfg.outputs[i].rtsPin, cfg.outputs[i].rxPin, uart);
                if (line >= 0)
                {
            ...
            Serial.printf("[DMX] out%d ready (RMT ch%d%s): uni=%d tx=%d mode=%d\n", i, rmtCh,
                          mode == OUTPUT_MODE_RDM_FULL ? "+RDM" : "", cfg.outputs[i].universe, cfg.outputs[i].txPin,
                          (int)mode);
        }
        else
            Serial.printf("[DMX] out%d RMT init FAILED\n", i);
    }
    if (!dmxReady)
        Serial.println("[DMX] no outputs enabled");
    else
        Serial.printf("[DMX] ready (monitor=out%d rdm=out%d)\n", monitorOut, rdmOut);
}
