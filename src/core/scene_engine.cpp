...
#endif
    if (g_scenes)
        memset(g_scenes, 0, sizeof(Scene) * MAX_SCENES);
}
...
bool sceneLoadNvs(int idx)
{
    ...
    if (!g_scenes)
        return false;
    Preferences p;
    if (!p.begin(SCENE_NS, true))
        return false;
    if (!p.isKey(sceneMetaKey(idx).c_str()))
    {
        p.end();
        return false;
    }
...
void sceneRecall(int presetIdx, uint16_t fadeMs, int outIdx)
{
    if (!g_scenes)
        return;
    if (presetIdx < 0 || presetIdx >= MAX_SCENES)
        return;
