...
int rdmDiscBranch(uint64_t lower, uint64_t upper, rdm_uid_t* found)
{
    ...
    {
        uint8_t pkt[64];
        int     len = rdmBuild(pkt, RDM_UID_BROADCAST_ALL, RDM_CC_DISC_COMMAND, RDM_PID_DISC_UNIQUE_BRANCH, pd, 12);
        rdmTx(pkt, len);
        uint8_t rx[48];
...
bool rdmMute(const rdm_uid_t& uid)
{
    ...
    for (int attempt = 0; attempt < 3; attempt++)
    {
        ...
        rdmTx(pkt, len);
        if (rdmReadResp(uid, resp, sizeof(resp), &rpdl, &ack))
            return true;
