...
static void routeFrameImpl(int universe, const uint8_t* data, uint16_t length, uint8_t startCode, uint32_t senderIp,
                           uint8_t proto, uint8_t priority)
{
    updateSender(senderIp, proto, (int16_t)universe, priority, data, length);
    bool matched = false;
    for (int i = 0; i < MAX_OUTPUTS; i++)
    {
        if (!cfg.outputs[i].enabled || portAddress(cfg.outputs[i]) != (uint16_t)universe)
            continue;
        ...
        mergeOutput(i);
        if (i == viewOutput())
            maybeLog(i, &dmxBufferState().buffers[i].data[1], 512, senderIp, proto);
        // Universe splitting: mirror to additional outputs via splitMask
        if (cfg.outputs[i].splitMask)
        {
            for (int j = 0; j < MAX_OUTPUTS; j++)
            {
                if (!(cfg.outputs[i].splitMask & (1 << j)))
                    continue;
                if (!cfg.outputs[j].enabled || j == i)
                    continue;
                dmxBufWriteBegin(j);
