...
void setup()
{
    ...
    while (!Serial && millis() < 3000)
        ;  // wait for serial monitor (debug builds)
    ...
    if (cfg.wiredPhy == WIRED_PHY_SPI)
        g_useEth = g_useEth && cfg.ethW5500;
#endif
    if (g_useEth)
    {
        startWiredEth();
    ...
    if (MDNS.begin(cfg.hostname.c_str()))
    {
        ...
        if (cfg.protocol != 0)
        {
            ...
            MDNS.addServiceTxt("e131", "udp", "universe", String(cfg.outputs[0].universe + 1).c_str());
        }
    }
...
void loop()
{
    ...
    if (artNet().artCfgDirty)
    {
        artNet().artCfgDirty = false;
        saveConfig();
    }
    if (artNet().bqDirty)
    {
        ...
        p.putUChar("bqpolicy", artNet().bqPolicy);
        p.end();
    }
